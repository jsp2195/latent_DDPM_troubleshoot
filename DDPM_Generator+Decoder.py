...
# Config / constants
# -----------------------------
DEFAULT_ENCODED_FEATURES = "encoded_features_CURVATURE.npy"
...
def build_ddpm_arrays(encoded_path: str, obb_path: str, scale_path: str, curv_path: str) -> Tuple[np.ndarray, np.ndarray, LabelNormStats, LatentNormStats]:
...
class DDPMTrainer:
    ...
    @tf.function
    def train_step(self, opt, batch_x, batch_c):
        ...
        with tf.GradientTape() as tape:
            pred = self.model(xt, t, batch_c, training=True)
    ...
    def sample(self, x_init, class_vec):
        ...
        def body(i, _x):
            ...
            pred = self.model(_x, t, class_vec, training=False)
            _x = self.reverse_step(_x, pred, t)
            return i - 1, _x
...
def train_ddpm(args) -> None:
    X, Y, lbl_stats, lat_stats = build_ddpm_arrays(args.encoded_features, args.obb_vectors, args.scale_coeffs, args.curvatures)
...
def get_pointcloud_loaders(path: str, batch_size: int, val_split: float, seed: int):
    ensure_file(path)
...
def train_ae(args) -> None:
    ...
    train_loader, val_loader, point_size = get_pointcloud_loaders(args.pointcloud_path, args.ae_batch_size, args.val_split, args.seed)
